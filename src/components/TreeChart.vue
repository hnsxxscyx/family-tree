	...
	<table v-if="treeData.name">
          ...
          extend: treeData.children && treeData.extend
			...
			>
				<div :class="{ node: true, hasMate: treeData.mate }">
					<div class="person" @click="clickNode(treeData, $event)">
						<div class="avat">
							<img
								:src="
                  treeData.image_url?treeData.image_url:(treeData.sex === '男'
                    ? 'man.png'
                    : 'woman.png')
                "
							>
						...
						<div class="birthday" v-if="treeData.birthday">{{ treeData.birthday }}</div>
					</div>
		...
		<tr v-if="treeData.children && treeData.extend">
			<td v-for="(children, index) in treeData.children" :key="index" colspan="2" class="childLevel">
				<TreeChart :json="children" @click-node="clickNode"/>
			</td>
		</tr>
	</table>
</template>
